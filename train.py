...
def fit(train_dl, valid_dl, G, loss_fn, g_opt, num_epochs, curr_epoch, device, D, d_opt, batch_size):
    ...
    for epoch in range(num_epochs):
        ...
        G.eval()
        with torch.no_grad():
            g_valid_loss = sum(loss_fn(G(batch['image'].to(device)), batch['fixation'].to(device)) for batch in valid_dl)
...
def main():
    ...
    # Start training 
    fit(TRAIN_DL, VALID_DL, G, LOSS_FN, g_opt, NUM_EPOCHS, CURR_EPOCH, device, D, d_opt, BATCH_SIZE)
